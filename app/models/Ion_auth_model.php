...
//		$default_group = $query;
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['user_group'].'.'.$this->join['users'].'='.$this->tables['users'].'.id',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['groups'], $this->tables['user_group'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$this->db->where_in($this->tables['groups'] . '.name', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['user_group'].'.'.$this->join['groups'], $group_ids);
			}
		}
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['user_group'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, ')
		                ->where($this->tables['user_group'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['user_group'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['user_group']);
	}
	...
	{
		$this->trigger_events('get_group_permission');

		// if no id was passed use the current users id
//		$group_id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['group_permission'].'.'.$this->join['permissions'].' as id, '.$this->tables['permissions'].'.key, '.$this->tables['permissions'].'.table_name, ')
			->where($this->tables['group_permission'].'.'.$this->join['groups'], $group_id)
			->join($this->tables['permissions'], $this->tables['group_permission'].'.'.$this->join['permissions'].'='.$this->tables['permissions'].'.id')
			->get($this->tables['group_permission']);
	}
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['user_group'], [$this->join['groups'] => $group_id]);
