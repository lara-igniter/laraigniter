...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->pagination_all_delimiters = (isset($this->pagination_all_delimiters)) ? $this->pagination_all_delimiters : array('<span>','</span>');
        $this->pagination_all_arrows = (isset($this->pagination_all_arrows)) ? $this->pagination_all_arrows : array('<<','>>');
    ...
    public function paginate($rows_per_page, $total_rows = null, $page_number = 1)
    {
        ...
        if ($page_number == 1) {
            ...
            $this->start_page = $this->pagination_all_delimiters[0].anchor($uri_string, $this->pagination_all_arrows[0], array('class' => 'page-link first')).$this->pagination_all_delimiters[1];
        }
        ...
        if ($page_number == 1) {
            ...
            $uri_string = implode('/', $uri_array);
            if($previous_page - 1) {
                $uri_string .= '?page=' . (int) $previous_page;
            }
            $this->previous_page = $this->pagination_delimiters[0].anchor($uri_string, $this->pagination_arrows[0], array('class' => 'page-link prev')).$this->pagination_delimiters[1];
        }

        unset($uri_array[$segments]);
        $uri_string = implode('/', $uri_array);
		$uri_string .= '?page=' . (int) $next_page;
        ...
        if (isset($total_rows) && (ceil($total_rows/$rows_per_page) == $page_number)) {
            ...
            unset($uri_array[$segments]);
            $uri_string = implode('/', $uri_array);
            $uri_string .= '?page=' . (int) $last;
            $this->end_page = $this->pagination_all_delimiters[0].anchor($uri_string, $this->pagination_all_arrows[1], array('class' => 'page-link last')).$this->pagination_all_delimiters[1];
        }
        ...
        if (isset($total_rows)) {
            if ($total_rows!=0) {
                ...
                for ($i = 1; $i <= $number_of_pages; $i++) {
                    ...
                    $uri_string = implode('/', $uri_array);
                    if ($page_number == $i) {
                        $pagination_delimiter = substr_replace($this->pagination_delimiters[0], 'active ', 11, 0);
                        $link_start = '<a href="javascript:;" class="page-link">';
                        $link_end = '</a>';
                        ...
                        $links .= ($i == 1) ? anchor($uri_string, $i, array('class' => 'page-link')) : anchor($uri_string . '?page=' .  $i, $i, array('class' => 'page-link'));
                        $links .= $this->pagination_delimiters[1];
                    }
                }
