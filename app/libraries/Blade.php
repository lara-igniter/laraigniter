...
class Blade
{
    ...
    {
        $path = array(FCPATH . 'resources/views/');         // your view file path, it's an array
        ...
        $compiler->directive('date', function ($expressions) {
            list($date, $format) = explode(',', $expressions);
            return "<?php echo date($format, strtotime($date)); ?>";
        });

        $compiler->directive('notempty', function ($collection) {
            return "<?php if(!empty($collection)): ?>";
        });

        $compiler->directive('endnotempty', function () {
            return '<?php endif; ?>';
        });
